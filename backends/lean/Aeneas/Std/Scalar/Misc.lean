...
theorem UScalar.ofNatCore_bv_lt_equiv {ty} (x y : Nat) (hx) (hy) :
  ...
  simp only [ofNatCore, UScalar.ofBitVec_toBitVec]
...
@[simp, scalar_tac_simps] theorem U8.val_mod_size_eq (x : U8) : x.toNat % U8.size = x.toNat := by
  apply Nat.mod_eq_of_lt; scalar_tac_preprocess; exact x.toNat_lt

@[simp, scalar_tac_simps] theorem U8.val_mod_size_eq' (x : U8) : x.toNat % 256 = x.toNat := by
  apply Nat.mod_eq_of_lt; exact x.toNat_lt_size
...
@[simp, scalar_tac_simps] theorem U16.toNat_mod_size_eq' (x : U16) : x.toNat % 65536 = x.toNat := by
  apply Nat.mod_eq_of_lt; exact x.toNat_lt
...
@[simp, scalar_tac_simps] theorem Usize.toNat_mod_size_eq (x : Usize) : x.toNat % Usize.size = x.toNat := by
  apply Nat.mod_eq_of_lt; scalar_tac_preprocess; exact x.toNat_lt_two_pow_numBits
