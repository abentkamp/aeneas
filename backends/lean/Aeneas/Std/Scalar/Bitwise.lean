...
def UScalar.shiftLeft {ty : UScalarTy} (x : UScalar ty) (s : Nat) :
  ...
  if s < ty.numBits then
    ok (UScalar.ofBitVec ty (x.toBitVec.shiftLeft s))
  else fail .integerOverflow

def UScalar.shiftRight {ty : UScalarTy} (x : UScalar ty) (s : Nat) :
  Result (UScalar ty) :=
  if s < ty.numBits then
    ok (UScalar.ofBitVec ty (x.toBitVec.ushiftRight s))
  else fail .integerOverflow
...
def IScalar.shiftLeft {ty : IScalarTy} (x : IScalar ty) (s : Nat) :
  Result (IScalar ty) :=
  if s < ty.numBits then
    ok (IScalar.ofBitVec ty (x.toBitVec.shiftLeft s))
  else fail .integerOverflow

def IScalar.shiftRight {ty : IScalarTy} (x : IScalar ty) (s : Nat) :
  Result (IScalar ty) :=
  if s < ty.numBits then
    ok (IScalar.ofBitVec ty (x.toBitVec.sshiftRight s))
  else fail .integerOverflow
...
Bitwise and
-/
def UScalar.and {ty} (x y : UScalar ty) : UScalar ty := UScalar.ofBitVec ty (x.toBitVec &&& y.toBitVec)

def IScalar.and {ty} (x y : IScalar ty) : IScalar ty := IScalar.ofBitVec ty (x.toBitVec &&& y.toBitVec)
...
Bitwise or
-/
def UScalar.or {ty} (x y : UScalar ty) : UScalar ty := UScalar.ofBitVec ty (x.toBitVec ||| y.toBitVec)

def IScalar.or {ty} (x y : IScalar ty) : IScalar ty := IScalar.ofBitVec ty (x.toBitVec ||| y.toBitVec)
...
Xor
-/
def UScalar.xor {ty} (x y : UScalar ty) : UScalar ty := UScalar.ofBitVec ty (x.toBitVec ^^^ y.toBitVec)

def IScalar.xor {ty} (x y : IScalar ty) : IScalar ty := IScalar.ofBitVec ty (x.toBitVec ^^^ y.toBitVec)
...
Not
-/
def UScalar.not {ty} (x : UScalar ty) : UScalar ty := UScalar.ofBitVec ty (~~~x.toBitVec)

def IScalar.not {ty} (x : IScalar ty) : IScalar ty := IScalar.ofBitVec ty (~~~x.toBitVec)
...
theorem UScalar.ShiftRight_spec {ty0 ty1} (x : UScalar ty0) (y : UScalar ty1)
  ...
  simp only [BitVec.ushiftRight_eq, UScalar.ofBitVec_toNat]
  simp only [HShiftRight.hShiftRight, BitVec.ushiftRight, toBitVec_toNat]
...
theorem UScalar.ShiftRight_IScalar_spec {ty0 ty1} (x : UScalar ty0) (y : IScalar ty1)
  ...
  simp only [BitVec.ushiftRight_eq, UScalar.ofBitVec_toNat, Nat.instShiftRight]
  simp only [IScalar.toNat, BitVec.toNat_ushiftRight, toBitVec_toNat, Nat.shiftRight_eq]
...
theorem UScalar.ShiftLeft_spec {ty0 ty1} (x : UScalar ty0) (y : UScalar ty1) (size : Nat)
  ...
  simp only [BitVec.shiftLeft_eq, UScalar.ofBitVec_toNat]
  simp only [toBitVec_toNat, BitVec.toNat_shiftLeft, ShiftLeft.shiftLeft, Nat.shiftLeft_eq']
