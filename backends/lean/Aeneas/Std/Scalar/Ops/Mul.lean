...
/-- Auxiliary `mul_equiv` without naming the error - kept private as a stepping stone. -/
private theorem IScalar.mul_equiv_aux {ty} (x y : IScalar ty) :
  match mul x y with
...
theorem IScalar.mul_equiv {ty} (x y : IScalar ty) :
  ...
  have hfail : ∀ e, mul x y = fail e → e = .integerOverflow := by
    ...
    simp only [mul, tryMk, Result.ofOption] at he
    split at he <;> simp_all
  have h := mul_equiv_aux x y
  split <;> rename_i hRes <;> rw [hRes] at h
  · exact h
  · exact ⟨ hfail _ hRes, h ⟩
  · exact h
