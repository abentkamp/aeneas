...
def UScalar.wrapping_add {ty} (x y : UScalar ty) : UScalar ty := UScalar.ofBitVec ty (UScalar.toBitVec x + UScalar.toBitVec y)

def IScalar.wrapping_add {ty} (x y : IScalar ty) : IScalar ty := IScalar.ofBitVec ty (IScalar.toBitVec x + IScalar.toBitVec y)
...
@[simp, bvify] theorem UScalar.wrapping_add_bv_eq {ty} (x y : UScalar ty) :
  (wrapping_add x y).toBitVec = UScalar.toBitVec x + UScalar.toBitVec y := by
...
@[simp, bvify] theorem IScalar.wrapping_add_bv_eq {ty} (x y : IScalar ty) :
  (wrapping_add x y).toBitVec = IScalar.toBitVec x + IScalar.toBitVec y := by
