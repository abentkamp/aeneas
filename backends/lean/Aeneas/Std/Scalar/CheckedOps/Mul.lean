...
theorem core.num.checked_mul_UScalar_bv_spec {ty} (x y : UScalar ty) :
  ...
  simp only [checked_mul_UScalar, hMul] at *
...
theorem «%S».checked_mul_bv_spec (x y : «%S») :
  ...
  simp_all only [«%S».checked_mul, UScalar.max, «%S».max, «%S».numBits]
...
theorem core.num.checked_mul_IScalar_bv_spec {ty} (x y : IScalar ty) :
  ...
  simp only [checked_mul_IScalar, hMul] at *
...
theorem «%S».checked_mul_bv_spec (x y : «%S») :
  ...
  simp_all only [IScalar.min, IScalar.max, «%S».min, «%S».max, «%S».numBits]
