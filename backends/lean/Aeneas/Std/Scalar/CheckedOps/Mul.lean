...
theorem «%S».checked_mul_bv_spec (x y : «%S») :
  ...
  cases h: core.num.checked_mul_UScalar x y <;> simp_all only [and_self, true_and] <;> trivial
...
theorem «%S».checked_mul_bv_spec (x y : «%S») :
  ...
  cases h: core.num.checked_mul_IScalar x y <;>
    simp_all only [not_false_eq_true, and_self, true_and] <;> trivial
