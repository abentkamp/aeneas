...
@[reducible] instance {α : Type u} : GetElem (Vec α) Usize α (fun a i => i.toNat < a.val.length) where
...
theorem alloc.vec.Vec.set_getElem!_eq α [Inhabited α] (x : alloc.vec.Vec α) (i : Usize) :
  x.set i x[i]! = x := by
  have h := @Vec.getElem!_Usize_eq α _ x i
  rw [h]
  simp only [Vec, Subtype.ext_iff, set_val_eq, List.set_getElem!]
...
namespace Tests
  example
    ...
    := by
    have hlen1 : slots1.length = slots.length + 1 := by
      have h := congr_arg List.length hEq; simp at h; exact h
    have hlenv1 := alloc.vec.Vec.len_val slots1
    have hlenv := alloc.vec.Vec.len_val slots
    omega
  ...
  example
    ...
    := by
    have hcoerce : (↑slots : List (List α)).length = slots.length := rfl
    have hlen_eq : slots.length = ↑capacity := by
      have hlen := alloc.vec.Vec.len_val slots
      have hnew := alloc.vec.Vec.len_val (alloc.vec.Vec.new (List α))
      simp only [alloc.vec.Vec.new, alloc.vec.Vec.length, List.length_nil] at hnew h2
      omega
    have hfact : (↑divisor : ℕ) ≤ (↑slots : List (List α)).length * (↑dividend : ℕ) := by
      rw [hcoerce, hlen_eq]; linarith [Hfactor.2.2.2]
