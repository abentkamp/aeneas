...
def core.array.TryFromSharedArraySlice.try_from
  ...
  Result (core.result.Result (Array T N) core.array.TryFromSliceError) := do
  if h: s.len = N then .ok (.Ok ⟨s.val, by
    exact (Slice.len_val s).symm.trans (congrArg USize.toNat h)⟩)
  else .ok (.Err ())
...
def core.array.TryFromMutArraySlice.try_from
  ...
  if h: s.len = N then
    let back (a : core.result.Result (Array T N) core.array.TryFromSliceError) : Slice T :=
      ...
      | _ => s
    ok ((.Ok ⟨ s.val, by exact (Slice.len_val s).symm.trans (congrArg USize.toNat h)⟩, back))
  else ok ((.Err (), fun _ => s))
...
theorem Array.index_mut_SliceIndexRangeToUsizeSlice {T : Type} {N : Usize}
  ...
  simp only [core.slice.index.SliceIndexRangeToUsizeSlice.index_mut,
    show (r.end : Usize) ≤ a.to_slice.length from by have h' := (UScalar.le_equiv r.end N).mp h; omega]
  refine ⟨?_, ?_, ?_⟩
  · simp [Array.to_slice]
  · simp [Slice.length]; exact (UScalar.le_equiv r.end N).mp h
...
theorem Array.index_mut_SliceIndexRangeFromUsizeSlice {T : Type} {N : Usize}
  ...
  simp only [core.slice.index.SliceIndexRangeFromUsizeSlice.index_mut,
    Slice.drop,
    show (r.start : Usize) ≤ a.to_slice.length from by have h' := (UScalar.le_equiv r.start N).mp h; omega]
  refine ⟨?_, ?_, ?_⟩
  · simp [Array.to_slice]
