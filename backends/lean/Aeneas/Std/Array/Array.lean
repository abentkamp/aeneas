...
def Array.empty (α : Type u) : Array α (Usize.ofNat 0) := ⟨ [], by simp [UScalar.ofNatCore, UScalar.ofBitVec, UScalar.toNat] ⟩
...
def Array.make {α : Type u} (n : Usize) (init : List α) (hl : init.length = n.toNat := by simp; try native_decide) :
  Array α n := ⟨ init, by apply hl ⟩

example : Array Int (Usize.ofNat 2) := Array.make (Usize.ofNat 2) [0, 1] (by native_decide)

example : Array Int (Usize.ofNat 2) :=
  let x := 0
  let y := 1
  Array.make (Usize.ofNat 2) [x, y] (by native_decide)

example : Result (Array Int (Usize.ofNat 2)) := do
  let x ← ok 0
  let y ← ok 1
  ok (Array.make (Usize.ofNat 2) [x, y] (by simp [UScalar.ofNatCore, UScalar.ofBitVec, UScalar.toNat]))
...
theorem Array.getElem!_Nat_eq {α : Type u} [Inhabited α] {n : Usize} (v : Array α n) (i : Nat) : v[i]! = v.val[i]! := by
  simp only [instGetElem?ArrayNatLtLengthValListEqToNat, List.getElem!_eq_getElem?_getD]; split <;> simp_all
  rfl
...
@[reducible] instance instGetElem?ArrayUsize {α : Type u} {n : Usize} : GetElem? (Array α n) Usize α (fun a i => i.toNat < a.val.length) where
...
@[simp, scalar_tac_simps, simp_lists_hyps_simps] theorem Array.getElem!_Usize_eq {α : Type u} [Inhabited α] {n : Usize} (v : Array α n) (i : Usize) : v[i]! = v.val[i.toNat]! := by
  simp only [instGetElem?ArrayUsize, List.getElem!_eq_getElem?_getD]
...
theorem Array.index_usize_spec {α : Type u} {n : Usize} [Inhabited α] (v: Array α n) (i: Usize)
  ...
  simp only [show v[i]? = v.val[i.toNat]? from rfl]
  split <;> simp_all
