...
/-- Expand a `∀ e : Error, P e` quantifier into a finite conjunction over `Error`'s constructors.
`step` and `mvcgen` add this (locally, not as a global `@[simp]`) to the simp set that cleans up
the hypotheses generated by `spec_of_spec_partial` / `spec_partial_to_mvcgen`. Once the `∀ e` is
expanded, each per-constructor conjunct `P .Cᵢ` reduces independently: a `match`-shaped `p_fail`
collapses via `iota`, an `e = c ∧ Q`-shaped one collapses via constructor injectivity, and
`true_and`/`and_true`/`and_self` then strip trivial or duplicated conjuncts. -/
theorem _root_.Aeneas.Std.Error.forall_iff (P : Error → Prop) :
    (∀ e, P e) ↔
      P .assertionFailure ∧ P .integerOverflow ∧ P .divisionByZero ∧
      P .arrayOutOfBounds ∧ P .maximumSizeExceeded ∧ P .panic ∧ P .undef := by
  refine ⟨fun h => ⟨h _, h _, h _, h _, h _, h _, h _⟩, ?_⟩
  rintro ⟨_, _, _, _, _, _, _⟩ e; cases e <;> assumption
