...
/- ## Signed add: exercises `not`-distribution over `∨` and conjunction-splitting.

The fail predicate is `e = overflow ∧ (x + y > I32.max ∨ x + y < I32.min)`. After
`step_fail_failEq_iff` we get `¬ (x + y > I32.max ∨ x + y < I32.min)`; `not_or`
distributes the negation; `not_lt` (after `gt_iff_lt`) pushes it through each
comparison; and `splitAndGoals` separates the resulting `↑x + ↑y ≤ I32.max ∧
I32.min ≤ ↑x + ↑y` into two distinct hypotheses on the generated `step_spec`. -/
