...
private def commonPushNotLemmas : Array Name :=
  #[``gt_iff_lt, ``ge_iff_le, ``not_or, ``not_lt, ``not_le, ``or_imp, ``imp_true_iff, ``not_true,
    ``true_implies, ``forall_and]

/-- If the failure postcondition `p_fail` of `thApp : spec_partial x p_ok p_fail p_div` is written
    as a `match` on the error of the shape `fun | Cᵢ => Pᵢ | … | _ => False`, return a proof of the
    same `spec_partial` but with `p_fail` replaced by the propositionally-equal canonical form
    `fun e => (e = C₁ ∧ P₁) ∨ … ∨ (e = Cₖ ∧ Pₖ)` (a disjunction over the non-`False` arms).

    This lets the downstream `*_fail_failEq_iff` / `forall_and` simp lemmas simplify the hypotheses
    generated by `spec_of_spec_partial` (for `step`) and `spec_partial_to_mvcgen` (for `mvcgen`)
    uniformly: they only know the `e = c ∧ P` shape, not a raw `match` (whose scrutinee `e` is a
    bound variable that `simp` will not case-split on). `Error`'s constructors are all nullary, so
    each arm `Pᵢ` is closed.

    Returns `thApp` unchanged if `p_fail` is not such a `match` (e.g. the `e = c ∧ P` form, or a
    constant), or if the conversion fails for any reason — the caller is then no worse off. -/
private def matchFailToEq (thApp : Expr) : MetaM Expr := do
  try
    let ty ← instantiateMVars (← inferType thApp)
    let .const specName specLvls := ty.getAppFn | return thApp
    unless specName == ``Aeneas.Std.WP.spec_partial do return thApp
    let args := ty.getAppArgs
    unless args.size == 5 do return thApp
    let p_fail := args[3]!
    let errTy := Expr.const ``Aeneas.Std.Error []
    -- For each constructor `Cᵢ`, reduce `p_fail Cᵢ` to its arm; keep the non-`False` arms. Bail
    -- unless every constructor exposes a reducible `match` (leaving non-`match` shapes alone).
    let mut keep : Array (Expr × Expr) := #[]
    for ctorName in (← getConstInfoInduct ``Aeneas.Std.Error).ctors do
      let ctor := Expr.const ctorName []
      let .reduced arm ← reduceMatcher? (p_fail.beta #[ctor]).headBeta | return thApp
      unless arm.isConstOf ``False do keep := keep.push (ctor, arm)
    -- Build `p_fail_eq := fun e => ⋁ᵢ (e = Cᵢ ∧ Pᵢ)` (or `fun _ => False` if there are no arms).
    let p_fail_eq ← withLocalDeclD `e errTy fun e => do
      let mut disjuncts : Array Expr := #[]
      for (ctor, arm) in keep do
        disjuncts := disjuncts.push (mkApp2 (.const ``And []) (← mkEq e ctor) arm)
      let body := match disjuncts.toList with
        | [] => .const ``False []
        | d :: ds => ds.foldl (fun acc x => mkApp2 (.const ``Or []) acc x) d
      mkLambdaFVars #[e] body
    -- Prove `p_fail = p_fail_eq` by `funext e; cases e <;> simp`, then transport `thApp` across it.
    let heqMVar ← mkFreshExprSyntheticOpaqueMVar (← mkEq p_fail p_fail_eq)
    let [g] ← heqMVar.mvarId!.apply (← mkConstWithFreshMVarLevels ``funext) | return thApp
    let (e, g) ← g.intro `e
    ...
    for sub in ← g.cases e do
      let (r, _) ← simpTarget sub.mvarId ctx (simprocs := simprocs)
      unless r.isNone do return thApp
    -- `motive := fun pf => spec_partial x p_ok pf p_div`; cast along `congrArg motive heq`.
    let motive ← withLocalDeclD `pf (← inferType p_fail) fun pf =>
      mkLambdaFVars #[pf] (mkAppN (.const specName specLvls) (args.set! 3 pf))
    mkEqMP (← mkCongrArg motive (← instantiateMVars heqMVar)) thApp
...
private def saveStepPartialSpecFromThm (ext : Extension) (attrKind : AttributeKind) (stx : Syntax)
  ...
  let newName ← forallTelescope ty fun fvars _ => do
    ...
    let thApp ← matchFailToEq (mkAppN thConst fvars)
