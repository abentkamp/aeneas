...
/-- If the failure postcondition `p_fail` of `thApp : spec_partial x p_ok p_fail p_div` is written
    ...
    same `spec_partial` but with `p_fail` replaced by the propositionally-equal disjunction
    `fun e => (e = C₁ ∧ p_fail C₁) ∨ … ∨ (e = Cₖ ∧ p_fail Cₖ)`, via `Error.post_eq_disj`.
    ...
    bound variable that `simp` will not case-split on). The disjunction keeps one arm per
    constructor — including the spurious `e = Cᵢ ∧ False` ones and the still-unreduced match arms
    `p_fail Cᵢ` — but those are all eliminated by that same downstream simp set (a `match` on a
    literal constructor reduces under `simp`).
...
private def canonicalizeFailPostcond (thApp : Expr) : MetaM Expr := do
  try
    ...
    let heq ← mkAppM ``Aeneas.Std.Error.post_eq_disj #[p_fail]
