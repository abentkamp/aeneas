...
/-- Simp lemmas shared by `simplifyStepHypotheses` and `simplifyMvcgenHypotheses`:
    unfold `>`/`≥` to `<`/`≤`, distribute `¬` over `∨`, push `¬` through `<` / `≤`
    on any `LinearOrder`. -/
private def commonPushNotLemmas : Array Name :=
  #[``gt_iff_lt, ``ge_iff_le, ``not_or, ``not_lt, ``not_le]
...
private def simplifyStepHypotheses (extraMVars : Array Expr) : MetaM Unit := do
  ...
  -- h_fail : ∀ e, ¬ p_fail e
  simplify extraMVars[0]! "hFail"
  -- h_div : ¬ p_div
  simplify extraMVars[1]! "hDiv"
...
theorem mvcgen_fail_failEq_iff {α : Type u} {Q : Std.Do.PostCond α postShape}
    {c : Error} {P : Prop} :
    (∀ e, (e = c ∧ P) → willFail e Q) ↔ (P → willFail c Q) :=
  ⟨fun h hP => h c ⟨rfl, hP⟩, fun h _ ⟨he, hP⟩ => he ▸ h hP⟩

theorem mvcgen_fail_False_iff {α : Type u} {Q : Std.Do.PostCond α postShape} :
    (∀ e, False → willFail e Q) ↔ True :=
  ⟨fun _ => trivial, fun _ _ h => h.elim⟩

theorem mvcgen_div_False_iff {P : Prop} :
    (False → P) ↔ True :=
...
private def simplifyMvcgenHypotheses (extraMVars : Array Expr) : MetaM Unit := do
  ...
  -- fail
  simplify extraMVars[2]! "hFail"
  -- div
  simplify extraMVars[3]! "hDiv"
