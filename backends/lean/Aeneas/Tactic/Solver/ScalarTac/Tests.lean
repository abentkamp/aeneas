...
example
  ...
  (s1 : U32)
  (c1 : Bool)
  (hConv1 : if xi.toNat + c0u.toNat > U32.max then s1.toNat = ↑xi + ↑c0u - U32.max - 1 ∧ c1 = true else s1.toNat = xi.toNat + c0u.toNat ∧ c1 = false)
  (s2 : U32)
  (c2 : Bool)
  (hConv2 : if s1.toNat + yi.toNat > U32.max then s2.toNat = ↑s1 + ↑yi - U32.max - 1 ∧ c2 = true else s2.toNat = s1.toNat + yi.toNat ∧ c2 = false)
  (c1u : U8)
